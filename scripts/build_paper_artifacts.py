...
def make_figures(features: pd.DataFrame, analytic: pd.DataFrame, pred: pd.DataFrame, years: np.ndarray, field_ids: np.ndarray, offsets: np.ndarray) -> None:
    ...
    regime_order = ["Routine core", "Isolated novelty", "Fragmented search", "Frontier completion"]
    regime_summary = (
        regimes.groupby("innovation_regime")
        .agg(breakthrough_top5=("breakthrough_top5", "mean"), future_cites_3y=("future_cites_3y", "mean"))
    ...
    bar_ax.bar(x, 100 * regime_summary["breakthrough_top5"], color=["steelblue", "darkorange", "gray", "darkred"])
    bar_ax.set_xticks(x)
    bar_ax.set_xticklabels(regime_order, rotation=22, ha="right")
    bar_ax.set_ylabel("Top 5% papers (%)")
    bar_ax.set_title("Regime-level breakthrough rates")
    for xi, value in zip(x, 100 * regime_summary["breakthrough_top5"]):
        bar_ax.text(xi, value + 0.15, f"{value:.1f}", ha="center", va="bottom", fontsize=8)
    fig.tight_layout()
    fig.savefig(FIGURES / "innovation_regimes.pdf")
    plt.close(fig)
    metric_labels = ["ROC AUC", "Average precision"]
    baseline = pred.loc[pred["model"] == "Baseline controls", ["auc", "average_precision"]].iloc[0].to_numpy()
    topology = pred.loc[pred["model"] == "Controls + topology", ["auc", "average_precision"]].iloc[0].to_numpy()
    x = np.arange(len(metric_labels))
    fig, (ax, gain_ax) = plt.subplots(1, 2, figsize=(8.4, 4.0), gridspec_kw={"width_ratios": [2.2, 1.0]})
    ax.bar(x - 0.18, baseline, width=0.36, label="Baseline controls")
    ax.bar(x + 0.18, topology, width=0.36, label="Controls + topology")
    ax.set_xticks(x)
    ax.set_xticklabels(metric_labels)
    ax.set_ylabel("Test-set performance")
    ax.set_ylim(0, max(0.8, float(max(baseline.max(), topology.max())) + 0.08))
    for xpos, values in ((x - 0.18, baseline), (x + 0.18, topology)):
        for xi, value in zip(xpos, values):
            ax.text(xi, value + 0.012, f"{value:.3f}", ha="center", va="bottom", fontsize=8)
    ax.legend(frameon=False, loc="upper right")
    gains = (topology - baseline) * 1_000
    gain_ax.axhline(0, color="black", linewidth=0.8)
    gain_ax.bar(x, gains, width=0.48, color=["steelblue", "darkorange"])
    gain_ax.set_xticks(x)
    gain_ax.set_xticklabels(["AUC", "AP"])
    gain_ax.set_ylabel(r"Topology gain ($10^{-3}$)")
    gain_ax.set_ylim(0, max(1.0, float(gains.max()) * 1.35))
    for xi, value in zip(x, gains):
        gain_ax.text(xi, value + 0.05, f"+{value:.1f}", ha="center", va="bottom", fontsize=8)
    fig.tight_layout()
    fig.savefig(FIGURES / "prediction_performance.pdf")
    plt.close(fig)
    make_persistence_figure(years, field_ids, offsets)
