...
\begin{abstract}
This paper studies whether the geometry and topology of scientific knowledge networks contain ex ante information about breakthrough innovation. Using the OGBN-MAG subset of the Microsoft Academic Graph, I construct a dynamic scientific knowledge complex from paper-topic assignments, citation links, authorship, venues, and publication years. The central object is a measure of topological opportunity: a paper's location relative to previously unconnected topic pairs, triangular boundaries in the prior knowledge complex, and negatively curved topic-pair bridges. Papers published between 2011 and 2016 are then linked to their three-year forward citations and classified as breakthrough papers when they fall in the top five percent of their publication-year citation distribution. The preliminary evidence shows that topological opportunity predicts future scientific impact even after controlling for authorship, topic breadth, references, year fixed effects, and venue-group fixed effects. Out-of-time prediction exercises also show that topology improves the classification of future breakthrough papers. The results suggest that breakthrough science is not only a function of research scale or centrality, but also of the high-order structure of the knowledge space in which researchers search.
\end{abstract}
...
The results are preliminary but encouraging. The topology-augmented model raises out-of-time ROC AUC from \AucBaseline{} to \AucTopology{}. Regression estimates show that the topology index is positively associated with future scientific impact, and component specifications indicate that both novel topic-pair formation and boundary-completion potential matter. These facts are consistent with a view of innovation in which high-value ideas emerge from structured gaps in the knowledge space, not merely from scale, popularity, or centrality.
...
The topological opportunity index is the average of standardized pairwise novelty, boundary-completion potential, and negative curvature:
\begin{equation}
TO_i = \frac{1}{3}\left[z(N_i) + z(B_i) + z(-\bar F_i)\right].
\end{equation}
The object is intentionally ex ante: all components are computed using only knowledge observed before the paper's publication year.
...
Table~\ref{tab:regressions} reports the main regression evidence. The topology index is positively associated with both log forward citations and the breakthrough indicator. Component specifications show that the result is not driven by a single mechanical variable. Novel topic-pair formation, boundary-completion potential, and negative curvature each capture different aspects of knowledge geometry.

\input{tables/prediction_table.tex}

Table~\ref{tab:prediction} and Figure~\ref{fig:prediction} report out-of-time prediction. Models are trained on papers from 2011--2014 and tested on 2015--2016 papers. Adding topology improves breakthrough classification relative to a baseline with standard controls and fixed effects.
