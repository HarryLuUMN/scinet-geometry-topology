...
Table~\ref{tab:prediction} and Figure~\ref{fig:prediction} report out-of-time prediction. Models are trained on papers from 2011--2014 and tested on 2015--2016 papers. The prediction exercise uses histogram gradient boosting so that topology can enter through threshold effects and feature interactions rather than only as a linear main effect. Adding topology produces a small but positive gain over the baseline in both ROC AUC and average precision. This should be read as an incremental forecasting result rather than a large black-box performance claim.
